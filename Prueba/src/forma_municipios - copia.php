...
<!-- TRAYENDO LOS DPEPARTAMENTOS DESDE LA TABLA tab_dptos-->
			<?php
			$sentencia = $base_de_datos->query('SELECT a.id_dpto, a.nom_dpto FROM tab_dptos a ORDER BY 2');
			$dptos = $sentencia->fetchAll(PDO::FETCH_OBJ);
			?>
			<div class="form-group">
				<label for="id_dpto">Id. Departamento</label>
<!--				echo '<script>', 'showMessage();', '</script>';-->
				<input required name="id_dpto" type="text" maxlength=2 minlength=2 id="id_dpto" placeholder="Departamento" class="form-control">
			</div>
			...
			<div class="form-group">
				...
				<input required name="nom_municipio" type="text" id="nom_municipio" placeholder="Nombre Dpto." class="form-control">
			</div>
